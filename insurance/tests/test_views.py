...
class LogPaymentProcessorResponseTests(APITestCase):
    def setUp(self):
    ...
    def test_log_payment_processor_response_success(self):
        ...
        self.assertEqual(response.data['message'], "Payment response logged successfully")

        # Check that the payment log was created
        payment_log = PaymentAttemptLog.objects.get(object_id=self.quote.id)
        self.assertEqual(payment_log.status, PaymentAttemptLog.Status.SUCCESS)
    ...
    def test_log_payment_processor_response_failure(self):
        ...
        # Check that the payment log was created
        payment_log = PaymentAttemptLog.objects.get(object_id=self.quote.id)
        self.assertEqual(payment_log.status, PaymentAttemptLog.Status.FAILURE)
